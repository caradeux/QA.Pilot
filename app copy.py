...
def generar_script_test_nuevo(url, instrucciones, headless=True, max_tiempo=600, capturar_pasos=False, browser='chrome', fullscreen=True):
    ...
    # Código para capturar pantallas por pasos usando pyautogui
...
browser = Browser(
    config=BrowserConfig(
        headless={headless_str},  # Usar parámetro dinámico
        # No especificar browser_binary_path para usar navegador built-in de Playwright
    )
)

{captura_pasos_funciones}

async def main():
    try:
        print("DEBUG: Iniciando test...")
        print(f"DEBUG: Configuración headless={{{headless_str}}}")
        anthropic_key = os.getenv('ANTHROPIC_API_KEY')
        ...
        agent = Agent(
            task="Navega a {url} y luego: {task}",
            llm=llm,
            browser=browser
        )
...
"""
    ...
    # Generar código para la captura de pasos (integrado en browser-use 0.2.6)
    pasos_capturas_codigo = ""
    if capturar_pasos and pasos_numerados:
        pasos_capturas_codigo = """
        # Las capturas se registran automáticamente durante la ejecución
        # usando las funciones de captura integradas en el script
        """

    # Código simplificado para pasos (integrado en browser-use 0.2.6)
    pasos_individuales_codigo = ""
    if capturar_pasos and pasos_numerados and len(pasos_numerados) >= 3:
        pasos_individuales_codigo = """
        # En browser-use 0.2.6, las capturas se registran automáticamente
        # durante el proceso de automatización del agente
        """

    try:
        # Validar navegador y convertir a tipo compatible con browser-use
        # Valores aceptados son 'chromium', 'firefox', 'webkit'
        browser_type = 'chromium'  # Valor por defecto
        
        if browser.lower() == 'firefox':
            browser_type = 'firefox'
        elif browser.lower() == 'edge':
            browser_type = 'webkit'  # Edge usa webkit en browser-use
        elif browser.lower() == 'safari':
            browser_type = 'webkit'
            
        print(f"DEBUG: Navegador seleccionado '{browser}' convertido a browser_type='{browser_type}' para browser-use")
        
        # Determinar si se debe incluir la funcionalidad de captura de pasos
        captura_pasos_funciones_final = captura_pasos_codigo if capturar_pasos else ""
        
        # Usar pasos individuales si es posible, de lo contrario usar pasos capturas
        pasos_final = pasos_individuales_codigo if pasos_individuales_codigo else pasos_capturas_codigo
        
        # Preparar variables para la plantilla (simplificado para browser-use 0.2.6)
        formatted_script = script_template.format(
            headless_str=str(headless),  # Usar True/False de Python
            task=sanitizar_instrucciones(instrucciones),
            url=url,
            captura_pasos_funciones=captura_pasos_funciones_final,
            fullscreen=str(fullscreen)  # Añadir el parámetro fullscreen
        )
    
        return formatted_script
    except Exception as e:
        print(f"ERROR en generar_script_test_nuevo: {str(e)}")
        print(traceback.format_exc())
        return f"# Error al generar script: {str(e)}"
