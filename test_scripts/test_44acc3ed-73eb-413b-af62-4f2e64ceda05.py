...
async def main():
    try:
        ...
        agent = Agent(
            task="Navega a https://www.mercadolibre.cl/ y luego: 1. Escribe 'smart Tv' en la barra de b?squeda y presiona Enter2. Haz clic en el segundo producto3. Ver detalle",
            llm=llm,
            browser=browser
        )
        ...
        await tomar_captura_navegador(agent, "Escribe 'smart Tv' en la barra de búsqueda y presiona Enter", paso_num=1)
        await tomar_captura_navegador(agent, "Haz clic en el segundo producto", paso_num=2)
        await tomar_captura_navegador(agent, "Ver detalle", paso_num=3)
