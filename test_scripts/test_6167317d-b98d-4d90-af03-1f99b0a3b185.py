...
# Función para tomar capturas usando Pixashot (requiere Pixashot.exe corriendo)
import os
import time
import re
import requests
...
async def tomar_captura_navegador(agent, descripcion, paso_num=None):
    try:
        ...
        # Determinar el título de la ventana (ajusta según navegador)
        titulo_ventana = "Mozilla Firefox"  # O "Chrome" si usas Chrome
        url = f"http://localhost:39393/window/capture?title={titulo_ventana}&format=png"
        ...
        if response.status_code == 200:
            ...
            print(f"❌ Error al capturar ventana: {response.status_code} {response.text}")
            return None
    except Exception as e:
        print(f"❌ Error al conectar con Pixashot: {e}")
        return None
